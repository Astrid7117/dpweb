     ...
     public function buscarPersonaPorNroIdentidad($nro_identidad)
{
    ...
    {
        $arr_usuarios = array();
        $consulta = "SELECT * FROM persona WHERE rol<>'3' AND rol<>'2'"; // Excluye clientes cliente y proveedor
        $sql = $this->conexion->query($consulta);
    ...
    public function verProveedores()
    {
        ...
        $consulta = "SELECT id, razon_social FROM persona WHERE rol = '3'";
        $sql = $this->conexion->query($consulta);
        while ($objeto = $sql->fetch_object()) {
    ...
    {
        $arr_usuarios = array();
        $consulta = "SELECT * FROM persona WHERE rol = '2'"; // Solo clientes 
        $sql = $this->conexion->query($consulta);
        while ($objeto = $sql->fetch_object()) {
            array_push($arr_usuarios, $objeto);
        }
    ...
    {
        $arr_usuarios = array();
        $consulta = "SELECT * FROM persona WHERE rol = '3'"; // Solo proveedores con todos los campos
        $sql = $this->conexion->query($consulta);
        while ($objeto = $sql->fetch_object()) {
            array_push($arr_usuarios, $objeto);
        }
